...
results <- iterate_hector(core = c_ssp245, metric = my_metric, params = param_values, criterion = criterion_co2_obs())
...
Here, we will use the scoring function `score_ramp` which requires the user to provide additional arguments for calculating Hector run scores (`w1` and `w2`). As differences between Hector runs and observed data values are computed, runs with differences \< `w1` will score 1.0, runs with differences \> `w2` will score 0. Model run scores will decrease in a linear fashion as differences diverge from `w1`.
...
The result of scored Hector runs shows which model runs closely resemble values from observed data (scores close to 1), and which deviate further from observed values (scores closer to 0). These scores will be used to weight model contribution to our probabilistic projections.
...
hector_126 <- scored_hector_runs(ini_126, ssp_name = "ssp126", 50, metric_global_tas, criterion_co2_obs())
hector_245 <- scored_hector_runs(ini_245, ssp_name = "ssp245", 50, metric_global_tas, criterion_co2_obs())
hector_370 <- scored_hector_runs(ini_370, ssp_name = "ssp370", 50, metric_global_tas, criterion_co2_obs())
hector_585 <- scored_hector_runs(ini_585, ssp_name = "ssp585", 50, metric_global_tas, criterion_co2_obs())
