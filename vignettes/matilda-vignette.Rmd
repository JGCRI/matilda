...
Metric objects contain a combination of identifiers that communicates what data is important for fetching from a Hector core. These objects are important for reducing computing time and memory by only saving the output we are interested in. For example, we can use a metric object defined as:
...
Below we will create a metric indicating that we want to retrieve results for global average air temperature anomaly (`global_tas`) for the years 2000 to 2100. We will also specify that we are interested in computing the _mean_ of this temperature output.
...
Running `iterate_hector()` requires a core, a **metric** object, a data frame of parameter values, and a **scoring criterion** object used for scoring Hector runs. 

Scoring criterion are objects used to aid in scoring model runs. A criterion defines information that should be fetched from the Hector core to score Hector runs against current observations. The `matilda` package provides internal criterion for easy use but also gives the user the ability to create their own scoring criterion using the `new_criterion()` function. For example:
...
Viewing the full results shows 10 separate runs (as indicated by `run_number`) for `global_tas` and `CO2_concentration` values from 2000-2100 and 1960-2021, respectively.
...
Similarly, here we score Hector runs based on how closely they reflect observed data; this score is then used to weight the runs in the final probabilistic calculation.

We can score Hector runs with the function `score_hruns()` which uses any **scoring function** to screen Hector results. A scoring function is a mathematical function that aims to weight model realizations based on proximity to observed data.

Here, we will use a `matilda` provided scoring function called `score_ramp`. As differences between Hector runs and observed data values are computed, runs with differences \< `w1` will score 1.0, runs with differences \> `w2` will score 0. Model run scores will decrease in a linear fashion as differences diverge from `w1` and toward `w2`.
...
We can establish a new metric to filter global_tas for years 2000-2100 from our `iterate_hector()` result, then we can use `metric_calc()` to compute the mean values for each run:
...
Using the newly calculated metric values and the scored model runs, we can to compute the likelihood of different climatic outcomes. The `prob_calc()` function allows us to group metric values into bins and calculates probabilities of each outcome, weighted by the scores of the Hector runs.
...
probabilities <- prob_calc(metric_values$metric_result, 
                           bins = bins, 
...
The overall procedure for this is the same as presented in the previous sections. The example results presented below we run each of four SSP scenarios () through 50 Hector simulations. We scored the Hector model realizations using observed CO~2~ concentrations.  
...
scored_hector_runs <- function(inifile, ssp_name, draws, metric, crit) {
  # initiate and core
  core = newcore(inifile, name = ssp_name)
  # generate parameters
  params = generate_params(core, draws)
  # running Hector
  h_result = iterate_hector(core, metric, crit, params)
  # score Hector runs 
...
metric_global_tas <- new_metric(var = GLOBAL_TAS(),
                                years = 1990:2100,
                                op = mean)

# new metric for Hector runs
metric_running_h <- new_metric(var = NULL,
                               years = 1990:2100,
                               op = mean)

# Running hector for each scenario
hector_126 <- scored_hector_runs(ini_126, ssp_name = "ssp126", 50, metric_running_h, criterion_co2_obs())
hector_245 <- scored_hector_runs(ini_245, ssp_name = "ssp245", 50, metric_running_h, criterion_co2_obs())
hector_370 <- scored_hector_runs(ini_370, ssp_name = "ssp370", 50, metric_running_h, criterion_co2_obs())
hector_585 <- scored_hector_runs(ini_585, ssp_name = "ssp585", 50, metric_running_h, criterion_co2_obs())
